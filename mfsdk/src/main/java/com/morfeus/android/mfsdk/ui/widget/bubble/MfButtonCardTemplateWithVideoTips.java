...
import com.morfeus.android.mfsdk.ui.widget.bubble.model.MfButtonCardTemplateWithVideoTipsModel;
...
public class MfButtonCardTemplateWithVideoTips extends TemplateView {
    public static final String BUTTON_CARD_WITH_VIDEO_TIPS_TEMPLATE_VIEW = "ButtonCardTemplateWithVideoTips";
    private static Context sContext;

    public MfButtonCardTemplateWithVideoTips(Context context) {
        super(context);
        sContext = context;
    }
    ...
    public void initView(Context context, AttributeSet attrs) {
        TemplateFactory.getInstance().registerTemplate(BUTTON_CARD_WITH_VIDEO_TIPS_TEMPLATE_VIEW, this);
    }

    @Override
    public TemplateView inflate(Context context) {
        return (MfButtonCardTemplateWithVideoTips) LayoutInflater.from(context)
                .inflate(R.layout.chat_item_button_card_with_video_tips_layout, this);
    }
    ...
    public BaseView create(Context context) {
        return new MfButtonCardTemplateWithVideoTips(context);
    }
    ...
    public static class ViewHolder extends TemplateViewHolder {
        ...
        private ImageView mIvVideoPlayImage;
        ...
        ViewHolder(View itemView) {
            ...
            this.mLLContainer = (LinearLayout) itemView.findViewById(
                    R.id.chat_item_btn_with_video_tips_layout_lin_container
            );
            ...
            this.mIvVideoPlayImage = (ImageView) itemView.findViewById(R.id.iv_play_image);
            this.mIvVideoImage = (ImageView) itemView.findViewById(R.id.iv_video_image);
            this.mTvVideoText = (TextView) itemView.findViewById(R.id.tv_video_text);
        ...
        public void setData(@NonNull TemplateModel model) {
            ...
            if (!(model instanceof MfButtonCardTemplateWithVideoTipsModel)) {
                throw new IllegalTemplateModelException(
                        "Error: Invalid template model!"
                );
            }
            MfButtonCardTemplateWithVideoTipsModel mfButtonCardTemplateWithVideoTipsModel = (MfButtonCardTemplateWithVideoTipsModel) model;
            applyDefaultCardStyle(mfButtonCardTemplateWithVideoTipsModel.isIncoming());


            if (mfButtonCardTemplateWithVideoTipsModel.isIncoming()) {
            ...
            List<MfListContentData> mfListContentDatas = mfButtonCardTemplateWithVideoTipsModel.getMfListContentDatas();
                ...
                switch (i) {
                    ...
                    case 2:
                        feedFooter(mfComponentDatas.get(0).getElementDataList(),
                                mfButtonCardTemplateWithVideoTipsModel);
                        break;
                }
        ...
        private void feedFooter(List<MfElementData> buttonsList,
                                MfButtonCardTemplateWithVideoTipsModel mfButtonCardTemplateWithVideoTipsModel) {
            ...
            if (buttonsList != null) {
                for (int i = 0; i < btnArray.length; i++) {
                    if (buttonsList.size() > i) {
                        updateButtonInfo(buttonsList.get(i), mfButtonCardTemplateWithVideoTipsModel,
        ...
        private void updateButtonInfo(final MfElementData elementData,
                                      final MfButtonCardTemplateWithVideoTipsModel mfButtonCardTemplateWithVideoTipsModel,
        ...
        private void feedImageInfo(List<MfComponentData> mfComponentDatas) {
            if (mfComponentDatas != null && mfComponentDatas.size() > 0) {
                ...
                if (mfElementData != null && mfElementData.getId().equals("title")) {
                    ...
                    int colorInt = Color.parseColor(color);
                    mTvVideoText.setText(title);
                    mTvVideoText.setTextColor(colorInt);
                    mTvVideoText.setVisibility(View.VISIBLE);
                } else {
            ...
            if (mfComponentDatas != null && mfComponentDatas.size() > 1) {
                ...
                final MfElementData mfElementData = mfComponentData.getElementData();
                if (mfElementData != null && mfElementData.getId().equals("video")) {
                    //TODO Need to add the code to download the image and set the snapshot of the same

                    if (mfElementData.getStyleData() != null) {
                        String playImage = mfElementData.getStyleData().getPlayImage();
                        if (!(TextUtils.isEmpty(playImage))) {
                            int resourceId = 0;
                            try {
                                resourceId = getResourceId(playImage);
                            } catch (Exception e) {
                                LogManager.e(TAG, e.getMessage());
                            }
                            if (resourceId != 0) {
                                mIvVideoPlayImage.setImageResource(resourceId);
                            } else {
                                mIvVideoPlayImage.setImageResource(R.drawable.play_btn);
                            }
                        }
                    }
        ...
        private void applyDefaultCardStyle(boolean incoming) {
            ...
            if (incoming) {
                styleTemplateId = BUTTON_CARD_WITH_VIDEO_TIPS_TEMPLATE_VIEW + "-" + ConfigParser.CardStyleKey.KEY_INCOMING;
            } else {
                styleTemplateId = BUTTON_CARD_WITH_VIDEO_TIPS_TEMPLATE_VIEW + "-" + ConfigParser.CardStyleKey.KEY_OUTGOING;
            }
