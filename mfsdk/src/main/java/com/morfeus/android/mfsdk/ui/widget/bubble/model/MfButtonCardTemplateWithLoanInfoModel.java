...
public class MfButtonCardTemplateWithLoanInfoModel extends TemplateModel {
    private List<MfListContentData> mfListContentDatas;

    public MfButtonCardTemplateWithLoanInfoModel(@NonNull String templateId, @NonNull MFCardData MFCardData) {
    ...
    private MfListContentData deserializeMsgDataCardContentsElement(JsonObject contentJsonObject) {
        ...
        if (elementJsonObject.has(MsgParser.KEY_GAUGE)) {
            JsonObject labelTwoElementJsonObject = elementJsonObject.getAsJsonObject(MsgParser.KEY_GAUGE);
            MfElementData mfElementData = deserializeElement(MsgParser.KEY_GAUGE, labelTwoElementJsonObject);
            MfComponentData mfComponentData = new MfComponentData(mfElementData);
            mfCardsList.add(mfComponentData);
        }
    ...
    private MfElementData deserializeElement(String id, JsonObject titleElementJsonObject) {
        ...
        if (titleElementJsonObject.has(MsgParser.KEY_SUB_TEXT)) {
            subText = titleElementJsonObject.get(MsgParser.KEY_SUB_TEXT).getAsString();
        }

        if (titleElementJsonObject.has(MsgParser.KEY_DISPLAY)) {
            display = titleElementJsonObject.get(MsgParser.KEY_DISPLAY).getAsString();
        }
        ...
        if (titleElementJsonObject.has(MsgParser.KEY_STYLE)) {
            ...
            if (elementStyle.has(MsgParser.KEY_FILL_COLOR)) {
                fillColor = elementStyle.get(MsgParser.KEY_FILL_COLOR).getAsString();
            }

            styleData = new MfElementStyleData.Builder().setTextColor(textColor).setFillColor(fillColor)
                    .setBackgroundColor(backgroundColor).build();
        }

        MfElementData mfElementData = new MfElementData.Builder(id, type).setText(text)
                .setSubText(subText).setDisplay(display)
