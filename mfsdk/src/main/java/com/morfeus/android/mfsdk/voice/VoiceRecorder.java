...
//        }
    ...
    /**
     * method to set the callback to null
     */
            ...
            while (true) {
                synchronized (mLock) {
                    ...
                    if (mBuffer != null && mBuffer.length > 0) {
                        for (byte aBuffer : mBuffer) {
                            volume += aBuffer * aBuffer;
                        }
                    } else {
                        end();
                        return;
                    ...
                    final float amplitude = volume * 1f / size;
//                    final int amplitude = (mBuffer[0] & 0xff) << 8 | mBuffer[1];
//                    double amplitudeDb = 20 * Math.log10((double) Math.abs(amplitude) / 32768);
                    ...
                    if (isHearingVoice(mBuffer, size)) {
                        ...
                        if (mCallback != null)
                            mCallback.onVoice(mBuffer, size, amplitude);
